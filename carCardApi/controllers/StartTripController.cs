...
namespace carCard.Controllers
{
    ...
    public StartTripController(ConnectionProvider connectionProvider,DataTableService dataTableService)
    {   
    ...
    public IActionResult GetCarsUsage()
    {
        ...
        try
        {
            ...
            {
                connection.Open();
                var query = "SELECT * FROM CARS_USAGE WHERE CAU_CAR_ID = @CarId ORDER BY CAU_DATE DESC;";
                using (var command = new SqlCommand(query, connection))
                {
